...
test("renders properly", async () => {
    mockFetchShow.mockResolvedValueOnce(datas);
    render(<App/>)
    await screen.findAllByAltText(/stranger things/i);
    expect(mockFetchShow).toHaveBeenCalled()
})

test("able to select season 1 from drop down and return 8 episodes", async () => {
    mockFetchShow.mockResolvedValueOnce(datas);
    render(<App/>)
    let dropdown = await screen.findByText(/select a season/i);
    userEvent.click(dropdown);
    userEvent.click(await screen.findByText(/season 1/i))
    expect(() => getAllByText(/minutes/i).toHaveLength(8))
})